...
export interface SceneCard {
    ...
    height?: number
    createdAt: number
    excludePinned?: boolean  // During rotation, skip pinned characters for this scene (e.g. solo/혼자 scenes)
}
...
export const useSceneStore = create<SceneState>()(
    persist(
        (set, get) => ({
            ...
            // Generation Actions - Round-robin with random batch size (1/10 ~ 1/5 of original queue)
            _rrSceneIndex: -1,
            _rrBatchRemaining: 0,
            _rrOriginalCounts: {} as Record<string, number>,
            decrementFirstQueuedScene: (presetId) => {
                const state = get() as any
                const preset = state.presets.find((p: any) => p.id === presetId)
                if (!preset) return null

                const scenes = preset.scenes
                const queued = scenes.filter((s: any) => s.queueCount > 0)
                if (queued.length === 0) return null

                // Check if current batch is still active
                const currentIdx = state._rrSceneIndex
                const currentScene = currentIdx >= 0 && currentIdx < scenes.length ? scenes[currentIdx] : null
                const batchDone = state._rrBatchRemaining <= 0
                const sceneDone = !currentScene || currentScene.queueCount <= 0

                if (batchDone || sceneDone) {
                    // Find next scene in order (round-robin)
                    let nextIdx = -1
                    const startIdx = currentIdx >= 0 ? currentIdx : -1
                    for (let i = 1; i <= scenes.length; i++) {
                        const idx = (startIdx + i) % scenes.length
                        if (scenes[idx].queueCount > 0) {
                            nextIdx = idx
                            break
                        }
                    }
                    ...
                    // Random batch size: between 1/10 and 1/5 of original queue (min 1)
                    const origCount = state._rrOriginalCounts[scenes[nextIdx].id] || scenes[nextIdx].queueCount
                    const minBatch = Math.max(1, Math.round(origCount / 10))
                    const maxBatch = Math.max(1, Math.round(origCount / 5))
                    const batchSize = minBatch + Math.floor(Math.random() * (maxBatch - minBatch + 1))
                    // Don't exceed remaining queue
                    const actualBatch = Math.min(batchSize, scenes[nextIdx].queueCount)

                    set({ _rrSceneIndex: nextIdx, _rrBatchRemaining: actualBatch - 1 } as any)
                    get().setQueueCount(presetId, scenes[nextIdx].id, scenes[nextIdx].queueCount - 1)
                    return scenes[nextIdx]
                }

                // Continue current batch
                set({ _rrBatchRemaining: state._rrBatchRemaining - 1 } as any)
                get().setQueueCount(presetId, currentScene.id, currentScene.queueCount - 1)
                return currentScene
            },
            ...
            initGenerationProgress: () => set(state => {
                const preset = state.activePresetId ? state.presets.find(p => p.id === state.activePresetId) : null
                const total = preset?.scenes.reduce((sum, s) => sum + s.queueCount, 0) || 0
                // Save original queue counts for round-robin batch calculation
                const origCounts: Record<string, number> = {}
                preset?.scenes.forEach(s => { if (s.queueCount > 0) origCounts[s.id] = s.queueCount })
                return {
                    completedCount: 0,
                    totalQueuedCount: total,
                    _rrSceneIndex: -1,
                    _rrBatchRemaining: 0,
                    _rrOriginalCounts: origCounts,
                } as any
            }),
...
// Auto-backup: snapshots persist payload to Pictures/NAIS_Backup/scenes/
// (5s debounce, 30 rotations). Survives IndexedDB corruption from hard crashes.
attachStoreBackup(useSceneStore as any, 'scenes')
