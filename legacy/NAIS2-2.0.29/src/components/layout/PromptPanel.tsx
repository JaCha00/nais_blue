...
export function PromptPanel() {
    ...
    // Subscribe to a DERIVED value (sum of queueCounts) so the generate button's
    // disabled state reactively updates when "전체 예약" / per-scene queue buttons
    // mutate scene.queueCount. Subscribing to the `getTotalQueueCount` function
    // reference alone never triggers a re-render — the function identity is stable.
    const sceneQueueCount = useSceneStore(state => {
        if (!state.activePresetId) return 0
    ...
    const handleGenerateOrCancel = useCallback(() => {
        ...
        if (isSceneMode) {
            // The main button is the global stop. An active rotation (running OR
            // paused) is stopped via stop(), which KEEPS the session so it can be
            // resumed later ("이어서 시작"); it does not discard progress.
            if (useRotationStore.getState().active) {
                useRotationStore.getState().stop('user clicked stop')
                return
            }
    ...
    }, [isConflict, isSceneMode, sceneIsGenerating, setSceneIsGenerating, isGenerating, cancelGeneration, generate, rotationActive])
    ...
    return (
        <div className="flex-1 flex flex-col h-full overflow-hidden p-2">
            ...
            <div className="p-0">
                ...
                <div className="flex gap-2">
                    <Button
                        variant={(isGenerating || (isSceneMode && (sceneIsGenerating || rotationActive))) ? "destructive" : "generate"}
                        size="lg"
                        className={cn(
                        ...
                        disabled={
                            (isSceneMode && sceneQueueCount === 0 && !sceneIsGenerating && !rotationActive) ||
                            isConflict
                        }
                    >
                        {isSceneMode ? (
