...
export function FragmentPromptDialog({ open, onOpenChange }: FragmentPromptDialogProps) {
    ...
    // txt 파일 불러오기 — uses a DOM file input instead of the Tauri native
    // open() dialog, which can fail to surface from inside a Radix modal /
    // borderless window (the "no response" bug). Reads via the browser FileReader.
    const txtInputRef = useRef<HTMLInputElement>(null)
    const jsonInputRef = useRef<HTMLInputElement>(null)

    const handleImportTxt = () => txtInputRef.current?.click()

    const onTxtFilesSelected = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        for (const file of files) {
            try {
                ...
                const fileName = file.name.replace(/\.txt$/i, '') || `import_${Date.now()}`
                ...
                console.error('Failed to read txt file:', file.name, err)
            }
        }
        if (importedCount > 0) {
            toast({
    ...
    // 전체 가져오기 (JSON) — DOM file input (see handleImportTxt note).
    const handleImportAll = () => jsonInputRef.current?.click()
                ...
                <div className="flex-1 flex gap-4 min-h-0">
                    ...
                    <div className="w-64 flex flex-col border rounded-lg">
                        <div className="p-2 border-b flex gap-1">
                            ...
                            {/* Hidden inputs for import (DOM file picker, reliable inside modals) */}
                            <input ref={txtInputRef} type="file" accept=".txt" multiple className="hidden" onChange={onTxtFilesSelected} />
                            <input ref={jsonInputRef} type="file" accept=".json" className="hidden" onChange={onJsonFileSelected} />
                            {/* Direct buttons (a DropdownMenu here doesn't open reliably inside the modal) */}
                            <Tip content={t('fragment.importTxt', 'txt 파일 불러오기')}>
                                <Button variant="ghost" size="sm" onClick={handleImportTxt}>
                                    <Upload className="h-4 w-4" />
                                </Button>
                            </Tip>
                            <Tip content={t('fragment.importAllJson', '전체 불러오기 (JSON)')}>
                                <Button variant="ghost" size="sm" onClick={handleImportAll}>
                                    <FileUp className="h-4 w-4" />
                                </Button>
                            </Tip>
                            <Tip content={t('fragment.exportAll', '전체 내보내기 (JSON)')}>
                                <Button variant="ghost" size="sm" onClick={handleExportAll} disabled={files.length === 0}>
