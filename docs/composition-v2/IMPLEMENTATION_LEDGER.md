...
| final process-local static link + Android build | 0 | 1 universal debug APK | Stronghold/libsodium/NAIS blue x86_64 Rust와 Gradle APK PASS; tracked binary 없음 |
| `npm run test:android-debug -- --apk ...` | 0 | 1 APK | retired development package ID, v2.8.1, minSdk 24, targetSdk 36, x86_64 verified |
...
Android exit-info는 NAIS blue를 native crash가 아닌 FontsProvider `DEPENDENCY DIED`로 분류했다.
...
| Source package | NAIS blue 2.8.1, Tauri Windows x64 |
...
5. Android logcat/dumpsys는 NAIS blue crash가 아닌 GMS privileged permission failure와 FontsProvider
   dependency death를 확인했다. NAIS blue manifest/runtime permission이나 Google Play dependency를
   추가하지 않았다.
...
`internal|privileged`였다. NAIS blue exit-info는 `reason=12 (DEPENDENCY DIED)`와
...
- Behavior added/changed: native data dirs exist before Stronghold initialization; close/relaunch awaits vault
  unload; I2I entry waits for vault readiness; GMS privileged permissions are rejected from NAIS blue manifest
...
3. Tracked Android plugin은 API 34+ UIDT와 API 24–33 foreground WorkManager, visible notification,
   ...
   default app process의 execution gate로 UIDT/WorkManager 동시 byte execution을 막는다. 이 Phase 12 선택은
   2026-07-17 dependency follow-up과 D-041에서 WorkManager 2.11.2/Kotlin 2.1.20으로 대체됐다.
...
따라서 이 Phase 12 시점의 최종 Kotlin compile/APK는 PASS가 아니다. Temporary generated include와 build logs는 제거했고 generated
source를 tracked authority로 추가하지 않았다.

2026-07-17 dependency follow-up은 generated source를 추적하지 않는 원칙을 유지하면서
`patch-android-signing.mjs`가 regenerated root Kotlin plugin을 2.1.20으로 정규화하도록 확장했다. WorkManager
2.11.2와 tracked transfer plugin의 `compileDebugKotlin`이 PASS했고 Android PR `source-contract`도 PASS했다.
전체 debug APK build/install은 같은 PR의 `debug-install` job을 authority로 사용한다.
