...
NAIS blue state/storage/log에 보관하지 않는다. Capability는 initialize, client create/load,
...
internal/privileged 보호 수준이며 NAIS blue는 `DEPENDENCY DIED`였다. 따라서 NAIS blue manifest/runtime
permission request나 try/catch로 해결할 수 없다. Contract는 이 privileged permission들을 NAIS blue에
...
상태: Accepted (executor gate open; dependency selection superseded by D-041)
...
## D-041 — Dependency resolution preserves Rust MSRV and normalizes generated Android Kotlin

- Date: 2026-07-17
- Decision: desktop R2 uses the AWS SDK `default-https-client` without the redundant legacy `rustls` feature, and
  Cargo resolver 3 must honor `rust-version = 1.88.0` for transitive selection. Android uses stable WorkManager 2.11.2;
  tracked preparation tooling normalizes Tauri's generated root Kotlin plugin to 2.1.20 after every init.
- Root cause: enabling both AWS HTTPS features retained `rustls 0.21`/`rustls-webpki 0.101.7` and three Dependabot
  alerts. Separately, WorkManager 2.11 metadata is Kotlin 2.1 while Tauri 2.11 generates Kotlin 1.9.25.
- Implementation: the vulnerable legacy TLS graph is removed from `Cargo.lock`; Resolver 3 prevents a normal update
  from selecting Rust 1.91+ Smithy crates. Android patching remains idempotent, fail-closed, covered by the release
  contract, and never makes generated files authoritative. CI moves from EOL Node 20 to Node 24 LTS.
- Alternatives rejected: retaining WorkManager 2.10.5 indefinitely; tracking generated Gradle files; suppressing Kotlin
  metadata checks; raising the Rust MSRV only to accept unrelated transitive updates; disabling Dependabot alerts.
- Dependency/bundle/mobile impact: npm compatible minor/patch updates and lockfiles are refreshed; weekly grouped
  Dependabot coverage tracks npm, Cargo, Gradle, and GitHub Actions. Major migrations remain separate review units.
