...
| R-029 | durable queue repository와 기존 workflow 사이 cutover가 아직 없다 | 이번 phase는 pure domain/repository만 만들고 enqueue/worker/UI를 의도적으로 연결하지 않았다 | High | 다음 승인 phase에서 workflow별 characterization과 shadow enqueue부터 진행하고 기존 Scene/generation-store 계약을 한 번에 교체하지 않음 | Open |
| R-030 | managed AppData resource producer가 아직 없다 | repository는 digest/ref와 missing-resource recovery를 검증하지만 source/mask bytes를 enqueue 시 managed storage로 복사하는 caller는 범위 밖이다 | High | producer가 없으면 volatile resource를 resumable로 표시하지 않고 explicit non-resumable 또는 blocked로 유지; future copy는 portable capability와 기존 image release 경계 재사용 | Open |
