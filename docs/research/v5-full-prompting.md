...
> 상태: Y01 시대감·배치 분리 실험 준비 · 최종 갱신: 2026-08-21
...
| Y01 | 다음 실행 | `year 2026`의 유무·가중치·헤더/하단 위치 | 시대감 효과와 전역 색조·밀도·마감 변화를 분리 |
...
동일한 기본·캐릭터 프롬프트와 동일한 Seed 세트를 사용한다. UC는 모두 비우고, 아래 한 줄의 유무와 위치만 바꾼다.
...
동일한 기본·캐릭터 프롬프트와 Seed 세트를 사용하고 한 번에 하나만 적용한다.
