...
> 상태: A02 전역 순서 모델 성립, LMN 장면 이식으로 반복성 추적 예정 · 최종 갱신: 2026-08-21
...
| 요소별 작가 귀속이 가능한가 | 현재 구조형 섹션명과 `scope` 자연어로는 불가능. 작가 태그는 지정 위치와 무관하게 전역으로 누출됨 |
...
6. `FACE STYLE` 같은 의미 라벨을 붙이지 않고, 전역 `ARTIST RECIPE`에 원하는 우선순서만 간결하게 적는다.
...
세 실험 모두 목표 요소 밖의 얼굴, 색, 명암, 구도와 배경이 함께 이동했다. 따라서 자연어 범위 지시는 작가 지문이 드러나는 위치에 약한 편향을 줄 수는 있어도 지역 마스크처럼 작동하지 않는다. 이후에는 요소별 작가 태그를 추가하지 않고, 작가 순서와 캐릭터의 의미 속성을 서로 독립적으로 관리한다.
