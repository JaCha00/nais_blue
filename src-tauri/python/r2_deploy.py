"""
Cloudflare R2 deployment orchestration for cleaned NAI Blue assets.

``tagger_server.py`` registers this router next to ``asset_postprocess.py`` so
the GUI can deploy the post-processed image and ``.nai-blue.json`` sidecar pair
...
class WranglerR2Uploader(BaseR2Uploader):
    ...
    async def probe(self, item: DeployItem) -> R2RemoteProbeResult:
        fd, temp_name = tempfile.mkstemp(prefix=".nai-blue-r2-probe.", suffix=".tmp")
        os.close(fd)
