package blue.bluehair.naiblue.transfer
...
class CloudflarePairingClient(private val context: Context) {
    ...
    fun configure(args: CloudflarePairingArgs): CloudflarePairingStatus {
        ...
        return try {
            ...
            connection.setRequestProperty("x-nai-blue-pairing-capability", args.pairingCapability)
...
private class CloudflareRequestClient(private val credentials: CloudflareCredentialStore) {
    ...
    fun signed(
        ...
        return try {
            ...
            connection.setRequestProperty("x-nai-blue-device", profile.deviceId)
            connection.setRequestProperty("x-nai-blue-sequence", sequence.toString())
            connection.setRequestProperty("x-nai-blue-nonce", nonce)
            connection.setRequestProperty("x-nai-blue-timestamp", timestamp.toString())
            connection.setRequestProperty("x-nai-blue-idempotency", idempotency)
            connection.setRequestProperty("x-nai-blue-content-sha256", bodyDigest)
            connection.setRequestProperty("x-nai-blue-signature", signature)
            partDigest?.let { connection.setRequestProperty("x-nai-blue-part-sha256", it) }
            write(connection, body)
            read(connection)
        } finally {
            connection.disconnect()
