...
function pairingDenial(stage: 'request' | 'capability' | 'expiry' | 'body' | 'consume' | 'install'): Response {
    console.warn(`NAI_BLUE_PAIRING_DENIED stage=${stage}`)
    return denial()
}
...
async function pair(request: Request, env: Env): Promise<Response> {
    ...
    const capability = request.headers.get('x-nai-blue-pairing-capability') ?? ''
...
export default {
    async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
        try {
            ...
            const deviceId = request.headers.get('x-nai-blue-device') ?? ''
...
export class TransferStateObject extends DurableObject<Env> {
    ...
    private async uploadPart(
        ...
        const declaredPartHash = request.headers.get('x-nai-blue-part-sha256') ?? ''
