...
export function parseSignedMetadata(headers: Headers): SignedRequestMetadata {
    const deviceId = headers.get('x-nai-blue-device') ?? ''
    const sequence = Number(headers.get('x-nai-blue-sequence'))
    const nonce = headers.get('x-nai-blue-nonce') ?? ''
    const timestampMs = Number(headers.get('x-nai-blue-timestamp'))
    const idempotencyKey = headers.get('x-nai-blue-idempotency') ?? ''
    const bodySha256 = headers.get('x-nai-blue-content-sha256') ?? ''
    const signature = headers.get('x-nai-blue-signature') ?? ''
