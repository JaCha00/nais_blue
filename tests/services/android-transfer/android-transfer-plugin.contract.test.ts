...
describe('Phase 12 Android transfer scheduling plugin contract', () => {
    ...
    it('uses UIDT on API 34+ and a foreground WorkManager fallback on API 24-33', async () => {
        const [scheduler, jobService, worker, notifications] = await Promise.all([
            source('android/src/main/java/blue/bluehair/naiblue/transfer/TransferScheduler.kt'),
            source('android/src/main/java/blue/bluehair/naiblue/transfer/NaiBlueTransferJobService.kt'),
            source('android/src/main/java/blue/bluehair/naiblue/transfer/NaiBlueTransferWorker.kt'),
            source('android/src/main/java/blue/bluehair/naiblue/transfer/TransferNotifications.kt'),
        ])
    ...
    it('persists only bounded secret-free tickets and exposes recovery controls', async () => {
        const [rustTypes, rustCommands, kotlinModel, repository, validator, plugin] = await Promise.all([
            ...
            source('android/src/main/java/blue/bluehair/naiblue/transfer/TransferTicket.kt'),
            source('android/src/main/java/blue/bluehair/naiblue/transfer/TransferTicketStore.kt'),
            source('android/src/main/java/blue/bluehair/naiblue/transfer/TransferTicketValidator.kt'),
            source('android/src/main/java/blue/bluehair/naiblue/transfer/AndroidTransferPlugin.kt'),
        ])
    ...
    it('keeps explicit single-owner guards across UIDT recovery and WorkManager', async () => {
        const [scheduler, worker, repository, plugin, execution, manifest] = await Promise.all([
            ...
            source('android/src/main/java/blue/bluehair/naiblue/transfer/TransferTicketStore.kt'),
            source('android/src/main/java/blue/bluehair/naiblue/transfer/AndroidTransferPlugin.kt'),
            source('android/src/main/java/blue/bluehair/naiblue/transfer/TransferExecution.kt'),
            source('android/src/main/AndroidManifest.xml'),
        ])
