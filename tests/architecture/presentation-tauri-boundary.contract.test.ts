...
describe('Presentation Tauri import baseline', () => {
    ...
    it('keeps native path API imports out of presentation', async () => {
        const modules = (await observedImports()).flatMap(entry => entry.modules)

        expect(modules).not.toContain('@tauri-apps/api/path')
    })

    it('keeps native key-value store imports out of presentation', async () => {
        const modules = (await observedImports()).flatMap(entry => entry.modules)
    ...
    it('keeps native binary writes out of migrated presentation flows', async () => {
        const files = (await observedImports()).map(entry => entry.file)

        expect(files).not.toContain('src/pages/SceneMode.tsx')
