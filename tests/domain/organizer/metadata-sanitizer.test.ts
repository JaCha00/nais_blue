...
function pngFixture(): Uint8Array {
    return new Uint8Array([
        ...
        ...pngChunk('apPx', [...ascii('app-specific')]),
        ...pngChunk('IDAT', [0]),
        ...pngChunk('IEND', []),
    ])
}
...
function jpegFixture(): Uint8Array {
    return new Uint8Array([
        ...
        ...jpegSegment(0xe3, [...ascii('third-party app metadata')]),
...
describe('Organizer strict metadata sanitization fixtures', () => {
    it('preserves removable container payloads for an audit sidecar before stripping', () => {
        const records = extractImageMetadataSegments(pngFixture())

        expect(records.map(record => record.type)).toEqual(['tEXt', 'zTXt', 'iTXt', 'eXIf', 'iCCP', 'apPx'])
        expect(atob(records[0].dataBase64)).toContain('metadata')
    })
    ...
    it('strips PNG EXIF/XMP/ICC/text/app chunks while retaining alpha-relevant critical data', () => {
        ...
        expect(scan.chunks).not.toEqual(expect.arrayContaining(['tEXt', 'zTXt', 'iTXt', 'eXIf', 'iCCP', 'apPx']))
    })
