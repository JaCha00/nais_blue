...
export interface PreservedImageMetadataSegment {
...
/** Captures exact removable container payloads before a sanitized copy is written. */
export function extractImageMetadataSegments(bytes: Uint8Array): PreservedImageMetadataSegment[] {
    const format = imageFormat(bytes)
    if (format === 'png') {
        return pngChunks(bytes)
            .filter(chunk => !shouldKeepPngChunk(chunk.type))
            .map(chunk => ({
                type: chunk.type,
                dataBase64: bytesToBase64(bytes.subarray(chunk.dataStart, chunk.dataStart + chunk.dataLength)),
            }))
    }
    if (format === 'webp') {
        return webpChunks(bytes)
            .filter(chunk => !WEBP_SAFE_CHUNKS.has(chunk.type))
            .map(chunk => ({
                type: chunk.type,
                dataBase64: bytesToBase64(bytes.subarray(chunk.dataStart, chunk.dataStart + chunk.dataLength)),
            }))
    ...
    if (format === 'jpeg') {
        return jpegSegments(bytes)
            .filter(segment => !shouldKeepJpegSegment(segment))
            .map(segment => ({
                type: segment.marker === 0xfe ? 'COM' : `APP${segment.marker - 0xe0}`,
                dataBase64: bytesToBase64(bytes.subarray(segment.payloadStart, segment.payloadStart + segment.payloadLength)),
            }))
    }
    return []
}
...
function stripWebpMetadata(bytes: Uint8Array): Uint8Array {
    ...
    const header: number[] = [...WEBP_SIGNATURE]
    writeU32Le(header, body.byteLength + 4)
    header.push(...WEBP_TAG)
    return concatenateBytes([new Uint8Array(header), body])
}
...
function stripJpegMetadata(bytes: Uint8Array): Uint8Array {
    ...
    for (const segment of jpegSegments(bytes)) {
        ...
        if (shouldKeepJpegSegment(segment)) parts.push(bytes.subarray(segment.start, segment.end))
        copiedTo = segment.end
    }
    parts.push(bytes.subarray(copiedTo))
    return concatenateBytes(parts)
}
