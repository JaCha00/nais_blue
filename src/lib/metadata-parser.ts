...
export interface NAIMetadata {
    /** NAI Blue contract. `version: 2` selects repository change-set apply. */
    naiBlue?: NaiBlueParams
    metadataVersion?: 1 | 2
    // Basic params
    prompt?: string
...
function applyNaiBlueOverlay(metadata: NAIMetadata, naiBlue: NaiBlueParams): NAIMetadata {
    metadata.naiBlue = naiBlue
    metadata.metadataVersion = naiBlue.version === 2 ? 2 : 1
    if (typeof naiBlue.qualityToggle === 'boolean') metadata.qualityToggle = naiBlue.qualityToggle
    if (typeof naiBlue.ucPreset === 'number') metadata.ucPreset = naiBlue.ucPreset

    if (naiBlue.promptParts) {
        metadata.promptParts = {
            base: naiBlue.promptParts.base ?? '',
            additional: naiBlue.promptParts.additional ?? '',
            detail: naiBlue.promptParts.detail ?? '',
            negative: naiBlue.promptParts.negative,
            inpainting: naiBlue.promptParts.inpainting,
            workflow: naiBlue.promptParts.workflow,
        }
        metadata.prompt = naiBlue.promptParts.base
        if (naiBlue.promptParts.negative !== undefined) {
            metadata.negativePrompt = naiBlue.promptParts.negative
        }
    }

    if (naiBlue.version === 2) {
        ...
        if (naiBlue.characters.length > 0) {
            metadata.v4_prompt = {
                caption: {
                    base_caption: naiBlue.promptParts.base,
                    char_captions: naiBlue.characters.map(character => ({
            ...
            metadata.v4_negative_prompt = {
                caption: {
                    base_caption: naiBlue.promptParts.negative ?? '',
                    char_captions: naiBlue.characters.map(character => ({
...
function metadataFromNaiBlue(naiBlue: NaiBlueParams): NAIMetadata {
    return applyNaiBlueOverlay({}, naiBlue)
}

export function parseNaiBlueSidecarMetadata(sidecar: Uint8Array | string): NAIMetadata | null {
    const naiBlue = readNaiBlueSidecar(sidecar)
    return naiBlue ? metadataFromNaiBlue(naiBlue) : null
}
...
async function extractTextChunkMetadata(bytes: Uint8Array): Promise<NAIMetadata | null> {
    ...
    // NAI Blue metadata is authoritative and may be the only metadata chunk when
...
export async function parseMetadataFromFile(file: File): Promise<NAIMetadata | null> {
    ...
    if (lowerName.endsWith('.nai-blue.json')
        ...
        return parseExternalMetadataJson(json) ?? parseNaiBlueSidecarMetadata(bytes)
    }
