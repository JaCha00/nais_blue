...
プロンプト読み込み領域へPNG、WebP、JPEG、`.nai-blue.json` sidecar、または対応するメタデータ抽出JSONをドロップします。メインとキャラクタープロンプトは同じ編集形式へ変換されます。他の自動化ツールから移行できるようNAIS2・NAIS3メタデータを読み込みますが、これらの識別子はインポート境界でのみ認識し、NAI Blueデータとして再保存しません。
